...
class droidInertial(MPU6050):
    ...
    def processInertial(self):
        ...
        if self.dataType == 'dmp':
            ...
            if self.newData == True:
                ...
                self.dmpAccel = (np.array([[linAccel.x / 2**14,linAccel.y / 2**14,linAccel.z / 2**14]])) * self.gravity         
    ...
    def storeIMUdata(self):
        temp = self.accel.tolist() + self.omega.tolist() +  [self.TimeK]
...
#         fig.legend(loc="upper right", bbox_to_anchor=(1,1), bbox_transform=ax.transAxes)
    ...
    def testLinProp(self):
        ...
        print('xGyro bias: %0.4f' % np.median(omega[:,0]))
        print('yGyro bias: %0.4f' % np.median(omega[:,1]))
        print('zGyro bias: %0.4f' % np.median(omega[:,2]))
        
        fig, ax = plt.subplots(5,3,figsize=(16,20))
        
        ax[0,0].plot(disp[:,0], 'r',label='Disp - x (m)')
        ax[0,0].set_ylabel('Disp- m')
        ax[0,1].plot(disp[:,1], 'r',label='Disp - y (m)')
        ax[0,2].plot(disp[:,2], 'r',label='Disp- z (m)')
        
        ax[1,0].plot(vel[:,0],'g',label='Vel - x (m/s)')
        ax[1,0].set_ylabel('Vel - (m/s)')
        ax[1,1].plot(vel[:,1],'g',label='Vel - y (m/s)')
        ax[1,2].plot(vel[:,2],'g',label='Vel - z (m/s)')
        ...
        ax[3,0].plot(theta[:,0], 'r',label='Rotation - x (rad)')
        ax[3,0].set_ylabel('Rotation -rad')
        ax[3,1].plot(theta[:,1], 'r',label='Rotation - y (rad)')
        ax[3,2].plot(theta[:,2], 'r',label='Rotation - z (rad)')
        
        ax[4,0].plot(omega[:,0], 'g',label='Ang Velocity - x (rad/s)')
        ax[4,0].set_ylabel('Ang Velocity -rad/s')
        ax[4,1].plot(omega[:,1], 'g',label='Ang Velocity - y (rad/s)')
        ax[4,2].plot(omega[:,2], 'g',label='Ang Velocity - z (rad/s)')
...
#     di = droidInertial(biasType = 'External',dataType = 'dmp')
